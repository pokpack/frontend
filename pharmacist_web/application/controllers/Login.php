...
//            $this->session->set_userdata(array('user_id' => $data->id, 'username' => $data->s_username, 'firstname' => $data->s_first_name , 'lastname' => $data->s_last_name, 'type_user' => $data->i_type));
            setcookie("phc_user_id", $data->id, time() + 3600, '/');
            setcookie("phc_username", $data->s_username, time() + 3600, '/');
            setcookie("phc_firstname", $data->s_first_name, time() + 3600, '/');
            setcookie("phc_lastname", $data->s_last_name, time() + 3600, '/');
            setcookie("phc_type_user", $data->i_type, time() + 3600, '/');
        }
